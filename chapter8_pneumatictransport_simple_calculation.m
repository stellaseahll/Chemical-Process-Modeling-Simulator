function varargout = chapter8_pneumatictransport_simple_calculation(varargin)
% CHAPTER8_PNEUMATICTRANSPORT_SIMPLE_CALCULATION MATLAB code for chapter8_pneumatictransport_simple_calculation.fig
%      CHAPTER8_PNEUMATICTRANSPORT_SIMPLE_CALCULATION, by itself, creates a new CHAPTER8_PNEUMATICTRANSPORT_SIMPLE_CALCULATION or raises the existing
%      singleton*.
%
%      H = CHAPTER8_PNEUMATICTRANSPORT_SIMPLE_CALCULATION returns the handle to a new CHAPTER8_PNEUMATICTRANSPORT_SIMPLE_CALCULATION or the handle to
%      the existing singleton*.
%
%      CHAPTER8_PNEUMATICTRANSPORT_SIMPLE_CALCULATION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CHAPTER8_PNEUMATICTRANSPORT_SIMPLE_CALCULATION.M with the given input arguments.
%
%      CHAPTER8_PNEUMATICTRANSPORT_SIMPLE_CALCULATION('Property','Value',...) creates a new CHAPTER8_PNEUMATICTRANSPORT_SIMPLE_CALCULATION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before chapter8_pneumatictransport_simple_calculation_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to chapter8_pneumatictransport_simple_calculation_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help chapter8_pneumatictransport_simple_calculation

% Last Modified by GUIDE v2.5 01-Jan-2014 10:18:05

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @chapter8_pneumatictransport_simple_calculation_OpeningFcn, ...
                   'gui_OutputFcn',  @chapter8_pneumatictransport_simple_calculation_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before chapter8_pneumatictransport_simple_calculation is made visible.
function chapter8_pneumatictransport_simple_calculation_OpeningFcn(hObject, eventdata, handles, varargin)
...
handles.verticalLength=getappdata(0,'verticalLength');
...
position(1)=[];
if (handles.verticalLength~=0||handles.numBends~=0)
...
function varargout = chapter8_pneumatictransport_simple_calculation_OutputFcn(hObject, eventdata, handles) 
...
if (hObject == handles.parametersVertical)
    axes(handles.plot);
    cla;
    showVerticalSimple(handles.size,handles.denF,handles.denP,handles.vis,handles.massP,handles.dia,handles.Ufs,handles.verticalLength,handles.isAcceleratedVertical,1);
else
    axes(handles.plot);
    cla;
    showVerticalSimple(handles.size,handles.denF,handles.denP,handles.vis,handles.massP,handles.dia,handles.Ufs,handles.verticalLength,handles.isAcceleratedVertical,2);
end
