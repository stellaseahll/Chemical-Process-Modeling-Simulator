function showEquationFluxNewton(size,denF,denP,vis,Ut,Re)
set(gca,'xtick',[]);
set(gca,'ytick',[]);
axis([-1,20,-30,3]);

showValues(size,denF,denP,vis);
equationNewton(size,denF,denP,vis,Ut,Re);
...
function equationNewton(size,denF,denP,vis,Ut,Re)
text('FontSize',15,'Position',[0 -8.5],'Interpreter','latex','String','$$\underline{Assume\hspace{2mm}Newton''s\hspace{2mm}Law\hspace{2mm}(Re_p>500)\hspace{5mm}}$$');
text('FontSize',15,'Position',[0 -10.5],'Interpreter','latex','String','$$Exponent\hspace{2mm}n=\hspace{2mm}\bf{2.4}$$');
