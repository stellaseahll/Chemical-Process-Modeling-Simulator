function showVerticalSimple(size,denF,denP,vis,massP,dia,Ufs,length,isAccelerated,step)
...
[Ptotal ,Pcomponent ,ev, U, f, ReCd]=findVerticalPipeConditions(massP,denP,denF,size,vis,dia,length,0,Ufs,~isAccelerated);
showValues(size,denF,denP,vis,massP,dia,length,Ufs,A,G);
if (step==1)
    showVoidFraction(Ufs,U,denP,G,ev,Ut,ReCd,f,denF,dia,vis);
elseif (step==2)
   showPressure(U,ev,f,denF,denP,Ufs,G,length,dia,isAccelerated,Pcomponent,Ptotal);
end
end
...
%pipe length
strLength=strcat('$$Vertical\hspace{2mm}Pipe\hspace{2mm}Length\hspace{2mm}(Total)\hspace{2mm}L_v=',sprintf('%.3f',length),'\hspace{1mm}m$$');
...
text('FontSize',13,'Position',[0 -11*1.1],'Interpreter','latex','String',strArea);
end

function showVoidFraction(Ufs,U,denP,G,ev,Ut,ReCd,f,denF,dia,vis)
text('FontSize',13,'Position',[0 -13*1.1],'Interpreter','latex','String','$$\underline{Determining\hspace{2mm}other\hspace{2mm}parameters\hspace{12mm}}$$');
%ev
text('FontSize',13,'Position',[0 -14*1.1],'Interpreter','latex','String','$$\bf{To\hspace{2mm}find\hspace{2mm}void\hspace{2mm}fraction\hspace{2mm}\epsilon_{H}:\hspace{10mm}}$$');
...
text('FontSize',13,'Position',[0 -15.5*1.1],'Interpreter','latex','String',strUpv);
text('FontSize',13,'Position',[0 -16.5*1.1],'Interpreter','latex','String',strUpv2);
text('FontSize',13,'Position',[0 -18*1.1],'Interpreter','latex','String',strUpv3);
text('FontSize',13,'Position',[0 -19.5*1.1],'Interpreter','latex','String',strUt);
text('FontSize',13,'Position',[0 -20.5*1.1],'Interpreter','latex','String',streh);
%Upv
text('FontSize',13,'Position',[0 -22.5*1.1],'Interpreter','latex','String','$$\bf{To\hspace{2mm}find\hspace{2mm}actual\hspace{2mm}particle\hspace{2mm}velocity\hspace{2mm}U_{pv}:\hspace{10mm}}$$');
streH1=strcat('$$U_{pv}=\frac{G}{\rho_p(1-\epsilon_v}=',...
    sprintf('\\frac{%.6f}{(%.3f)(1-%.4f)\\hspace{3mm}}=\\bf{%.4f\\hspace{1mm}m/s}$$',G,denP,ev,U(2)));
text('FontSize',13,'Position',[0 -24*1.1],'Interpreter','latex','String',streH1);
%Uf
text('FontSize',13,'Position',[0 -26*1.1],'Interpreter','latex','String','$$\bf{To\hspace{2mm}find\hspace{2mm}actual\hspace{2mm}fluid\hspace{2mm}velocity\hspace{2mm}U_{fH}:\hspace{10mm}}$$');
strUfv=strcat('$$U_{fv}=\frac{U}{\epsilon_v}=',...
    sprintf('\\frac{%.5f\\hspace{1mm}}{%.4f}=\\bf{%.5f\\hspace{1mm}m/s}$$',Ufs,ev,U(1)));
text('FontSize',13,'Position',[0 -27.5*1.1],'Interpreter','latex','String',strUfv);
%ff
text('FontSize',13,'Position',[0 -29.5*1.1],'Interpreter','latex','String','$$\bf{To\hspace{2mm}find\hspace{2mm}fluid\hspace{2mm}friction\hspace{2mm}factor\hspace{2mm}f_g:\hspace{10mm}}$$');
...
strFg3=sprintf('$$Substituting\\hspace{2mm}Re=%.4f\\hspace{2mm}and\\hspace{2mm}using\\hspace{2mm}trial\\hspace{2mm}and\\hspace{2mm}error,\\hspace{2mm}\\bf{f_g=%.5f}$$',...
    ReCd,f);
text('FontSize',13,'Position',[0 -31*1.1],'Interpreter','latex','String',strFg1);
text('FontSize',13,'Position',[0 -33*1.1],'Interpreter','latex','String',strFg2);
text('FontSize',13,'Position',[0 -35*1.1],'Interpreter','latex','String',strRe);
text('FontSize',13,'Position',[0 -37*1.1],'Interpreter','latex','String',strFg3);
end


function showPressure(U,ev,f,denF,denP,Ufs,G,length,diameter,isAccelerated,Pcomponent,Ptotal)
text('FontSize',13,'Position',[0 -13*1.1],'Interpreter','latex','String','$$\underline{Parameters\hspace{2mm}calculated\hspace{8mm}}$$');
%% State Values
%Ufv
strUfv=strcat('$$Actual\hspace{2mm}Fluid\hspace{2mm}Velocity\hspace{2mm}U_{fv}=',sprintf('%.5f',U(1)),'\hspace{1mm}m/s$$');
text('FontSize',13,'Position',[0 -14*1.1],'Interpreter','latex','String',strUfv);
%Upv
strUpv=strcat('$$Actual\hspace{2mm}Particle\hspace{2mm}Velocity\hspace{2mm}U_{pv}=',sprintf('%.5f',U(2)),'\hspace{1mm}m/s$$');
text('FontSize',13,'Position',[0 -15*1.1],'Interpreter','latex','String',strUpv);
%ev
strev=strcat('$$Void\hspace{2mm}Fraction\hspace{2mm}\epsilon_H=',sprintf('%.4f',ev),'$$');
text('FontSize',13,'Position',[0 -16*1.1],'Interpreter','latex','String',strev);
%fv
strfg=strcat('$$Fluid\hspace{2mm}Friction\hspace{2mm}Factor\hspace{2mm}f_g=',sprintf('%.5f',f),'$$');
text('FontSize',13,'Position',[0 -17*1.1],'Interpreter','latex','String',strfg);
%% title
...
strGp=sprintf('$$(-\\Delta p)_{grav,p}\\hspace{1mm}=\\rho_p(1-\\epsilon_v)gL_v=%.3f(1-%.4f)(9.81)(%.3f)\\hspace{1mm}=\\hspace{1mm}\\bf{%.5f\\hspace{1mm}Pa}$$',...
    denP,ev,length,Pcomponent(5));
strGf=sprintf('$$(-\\Delta p)_{grav,f}\\hspace{1mm}=\\rho_f(epsilon_v)gL_v=%.3f(%.4f)(9.81)(%.3f)\\hspace{1mm}=\\hspace{1mm}\\bf{%.5f\\hspace{1mm}Pa}$$',...
    denF,ev,length,Pcomponent(6));
%% pressure drop
if (isAccelerated)
